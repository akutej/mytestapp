...
job_all_answers <- (nrow(df.all))
job_admin_pers <- length(unique(df.all$ACC2SURV_ACCID[answerstable$Berufsgruppe == "Administration"]))
job_admin_answers <- sum(df.all$Berufsgruppe == "Administration")
job_care_pers <- length(unique(df.all$ACC2SURV_ACCID[answerstable$Berufsgruppe == "caregiver"]))
job_care_answers <- sum(df.all$Berufsgruppe == "caregiver")
job_manage_pers <- length(unique(df.all$ACC2SURV_ACCID[answerstable$Berufsgruppe == "management"]))
job_manage_answers <- sum(df.all$Berufsgruppe == "management")
job_medic_pers <- length(unique(df.all$ACC2SURV_ACCID[answerstable$Berufsgruppe == "medical"]))
job_medic_answers <- sum(df.all$Berufsgruppe == "medical")
job_tech_pers <- length(unique(df.all$ACC2SURV_ACCID[answerstable$Berufsgruppe == "technician"]))
job_tech_answers <- sum(df.all$Berufsgruppe == "technician")

job_all_hitImp <- as.integer(sum(df.all$IMPACT == df.all$middleIGRID, na.rm = TRUE))
job_admin_hitImp <- as.integer(sum(df.all$IMPACT[df.all$Berufsgruppe == "Administration"] == df.all$middleIGRID[df.all$Berufsgruppe == "Administration"], na.rm = TRUE))
job_care_hitImp <- as.integer(sum(df.all$IMPACT[df.all$Berufsgruppe == "caregiver"] == df.all$middleIGRID[df.all$Berufsgruppe == "caregiver"], na.rm = TRUE))
job_manage_hitImp <- as.integer(sum(df.all$IMPACT[df.all$Berufsgruppe == "management"] == df.all$middleIGRID[df.all$Berufsgruppe == "management"], na.rm = TRUE))
job_medic_hitImp <- as.integer(sum(df.all$IMPACT[df.all$Berufsgruppe == "medical"] == df.all$middleIGRID[df.all$Berufsgruppe == "medical"], na.rm = TRUE))
job_tech_hitImp <- as.integer(sum(df.all$IMPACT[df.all$Berufsgruppe == "technician"] == df.all$middleIGRID[df.all$Berufsgruppe == "technician"], na.rm = TRUE))


job_all_hitOcc <- as.integer(sum(df.all$OCCURRENCE == df.all$middleOGRID, na.rm = TRUE))

job_admin_hitOcc <- as.integer(sum(df.all$OCCURRENCE[df.all$Berufsgruppe == "Administration"] == df.all$middleOGRID[df.all$Berufsgruppe == "Administration"], na.rm = TRUE))
job_care_hitOcc <- as.integer(sum(df.all$OCCURRENCE[df.all$Berufsgruppe == "caregiver"] == df.all$middleOGRID[df.all$Berufsgruppe == "caregiver"], na.rm = TRUE))
job_manage_hitOcc <- as.integer(sum(df.all$OCCURRENCE[df.all$Berufsgruppe == "management"] == df.all$middleOGRID[df.all$Berufsgruppe == "management"], na.rm = TRUE))
job_medic_hitOcc <- as.integer(sum(df.all$OCCURRENCE[df.all$Berufsgruppe == "medical"] == df.all$middleOGRID[df.all$Berufsgruppe == "medical"], na.rm = TRUE))
job_tech_hitOcc <- as.integer(sum(df.all$OCCURRENCE[df.all$Berufsgruppe == "technician"] == df.all$middleOGRID[df.all$Berufsgruppe == "technician"], na.rm = TRUE))
...
job_df <- data.frame(
  ...
  administration = c(job_admin_pers,job_admin_answers,job_admin_hitImp,job_admin_hitOcc,job_admin_perhitImp,job_admin_perhitOcc,job_admin_UncImp,job_admin_UncOcc),
  caregiver = c(job_care_pers,job_care_answers,job_care_hitImp,job_care_hitOcc,job_care_perhitImp,job_care_perhitOcc,job_care_UncImp,job_care_UncOcc),
  management = c(job_manage_pers,job_manage_answers,job_manage_hitImp,job_manage_hitOcc,job_manage_perhitImp,job_manage_perhitOcc,job_manage_UncImp,job_manage_UncOcc),
  medical = c(job_medic_pers,job_medic_answers,job_medic_hitImp,job_medic_hitOcc,job_medic_perhitImp,job_medic_perhitOcc,job_medic_UncImp,job_medic_UncOcc),
  technician = c(job_tech_pers,job_tech_answers,job_tech_hitImp,job_tech_hitOcc,job_tech_perhitImp,job_tech_perhitOcc,job_tech_UncImp,job_tech_UncOcc),
  overall = c(job_all_pers,job_all_answers,job_all_hitImp,job_all_hitOcc,job_all_perhitImp,job_all_perhitOcc,job_all_UncImp,job_all_UncOcc)
)
...
answerstable$hitImp <- answerstable$IMPACT == answerstable$middleIGRID
answerstable$hitOcc <- answerstable$OCCURRENCE == answerstable$middleOGRID
