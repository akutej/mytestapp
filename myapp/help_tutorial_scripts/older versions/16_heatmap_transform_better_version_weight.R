...
for (anz in 1:numberscenarios) {
  ...
  Overall.Scenario.Perimeter.Uncertainty <- sum(df$uncertaintyPerimeterPixel)
  max.Scenario.Perimeter.Uncertainty <- max(df$uncertaintyPerimeterPixel)
  min.Scenario.Perimeter.Uncertainty <- min(df$uncertaintyPerimeterPixel)
  diffmaxmin.Scenario.Perimeter.Uncertainty <- max.Scenario.Perimeter.Uncertainty - min.Scenario.Perimeter.Uncertainty
  ...
  print(max.Scenario.Perimeter.Uncertainty)
  print(min.Scenario.Perimeter.Uncertainty)
  print (diffmaxmin.Scenario.Perimeter.Uncertainty)
  sum.Occ <- 0
  sum.Imp <- 0
  sum.Perimeter <- 0
  
  for (m in 1:numberofanswers){
    Uncertainty.Perimeter <- df[m,"uncertaintyPerimeterPixel"]
    ...
    distance.Perimeter <- (max.Scenario.Perimeter.Uncertainty-Uncertainty.Perimeter)  #Abstand zu maxWert Impact
    #sum.Occ <- sum.Occ + distance.Occ
    #sum.Imp <- sum.Imp + distance.Imp
    sum.Perimeter <- sum.Perimeter + distance.Perimeter
  }  
  #print (sum.Imp)
  #print (sum.Occ)
  print (sum.Perimeter)
...
for (i in 1:numberofanswers){
  ...
  UncertaintyPerimeterUser <- df[i,"uncertaintyPerimeterPixel"]
  ...
  distancePerimeterUser <- (max.Scenario.Perimeter.Uncertainty-UncertaintyPerimeterUser)
  Uncertaintyweight <- distancePerimeterUser / sum.Perimeter
  ...
  D[index,"Zähler"] <- D[index,"Zähler"] + (Uncertaintyweight*3)
 print (AccId)
  print (Uncertaintyweight*3)
  
}
  print (Uncertaintyweightall)
  #print (D)
  scenfile <- (paste0("myapp/files/4_heatmap/",scentext,"_transformed_new_weight_perimeter.xlsx"))
  scenpic <- (paste0("myapp/pictures/17_heatmap_pixel_graphic/",scentext,"_heatmap_weight_perimeter.bmp"))
  ...
  write.csv(D, paste0("myapp/files/4_heatmap/", scentext,"_transformed_new_weight_perimeter.csv"), row.names=TRUE)
