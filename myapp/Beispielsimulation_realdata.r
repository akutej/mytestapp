...
answerstable1 <- read.csv(file = 'myapp/Data/RQ1.csv', header=TRUE) #importiere das answers file
dfnogroup <- answerstable1 %>% filter(QUES2SURV_METHOD == "classic" & ANS2SURV_ANSWERED == 1 & QUES_ID == 281 )


>>>>>>> cc1b6233ff940a2564fa6de511509ba078523dee
...
# Monte-Carlo-Simulation
  ...
  for(run in 1:Runs){
    ...
    # Ziehung eines zufälligen Probanden aus dem Datensatzes
      Probanden <- Daten[,"ACC2SURV_ACCID"]  #,"ANS2SURV_ID"]
	...
	# Ermittlung des Auswirkungsintervalls des zufällig gezogenen Probandens
	  ...
	  untere.grenze <- Daten[i.proband,"X1PCT"]
	  obere.grenze <- Daten[i.proband,"X2PCT"]
...
# zur Auswirkung wiederspiegelt
  ...
  for(run in 1:Runs){
    ...
    # Ziehung einer zufälligen Auswirkung gemäß der individuellen Beta-Verteilung des Probanden	  
    x <- rbeta(1,c.alpha,c.beta)
    auswirkung2 <- (obere.grenze-untere.grenze)*x+untere.grenze
    
    # Zum Schluss wird noch die in diesem Run ermittelte Auswirkung abgespeichert
    Auswirkungen2[run] <- auswirkung2	
  ...
  Runs <- 10000     # die Anzahl an Durchläufen
  Eintrittswahrscheinlichkeit <- rep(NA,Runs)
  for(run in 1:Runs){
    ...
    # Ermittlung des Auswirkungsintervalls des zufällig gezogenen Probandens
    i.proband <- which(Probanden==proband)
    untere.grenze <- Daten[i.proband,"Y1PCT"]
    obere.grenze <- Daten[i.proband,"Y2PCT"]
    
    # Ziehung einer zufälligen Auswirkung gemäß der individuellen Beta-Verteilung des Probanden	  
    x <- rbeta(1,c.alpha,c.beta)
    eintritt <- (obere.grenze-untere.grenze)*x+untere.grenze
    
    # Zum Schluss wird noch die in diesem Run ermittelte Auswirkung abgespeichert
    Eintrittswahrscheinlichkeit[run] <- eintritt
  ...
  hist(Eintrittswahrscheinlichkeit)
...
# Daraus lässt sich nun die mittlere Auswirkung über alle Probanden angeben:
  ...
  mean(Eintrittswahrscheinlichkeit)
  median(Eintrittswahrscheinlichkeit)
  
  
# und ein Maße für die Unsicherheit angeben:
  sd(Auswirkungen)
  IQR(Auswirkungen)
  sd(Eintrittswahrscheinlichkeit)
  IQR(Eintrittswahrscheinlichkeit)
...
# Zum Beispiel lässt sich ein 90%-Prozent-Quantil angeben. 
  ...
  quantile(Eintrittswahrscheinlichkeit,0.9)
