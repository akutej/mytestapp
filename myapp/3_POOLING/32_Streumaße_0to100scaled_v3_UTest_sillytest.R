...
for (anz in 1:1){#number.scenarios) {
  ...
  for (i in 1:numberofanswers){
    ...
    while (actualvaluex <= x.max){
      ...
      weight.transformedx <- ((actualvaluex)*weight.IMPACT.value)
      #print (transformedx)
    ...
    while (actualvaluey <= y.max){
      ...
      weight.transformedy <- ((actualvaluey)*weight.OCCURRENCE.value)
  ...
  weight_gr_Q1_O <- quantile(weight.LIKELIHOOD, 0.25)[[1]]
  weight_gr_Q3_O <- quantile(weight.LIKELIHOOD, 0.75)[[1]]
