...
for (anz in 1:number.scenarios) {
  ...
  actual.df <- all.answers %>% filter(QUES2SURV_METHOD == "classic" & ANS2SURV_ANSWERED == 1 & QUES_ID == actualscenario)# & ACC2SURV_ACCID == "22")
...
#Graphischer Wert über Flächen & REACHED GRID TEIL & WEIGHT VARIANTE
  ...
  for (i in 1:numberofanswers){
    ...
    x_user_werte <- seq(x.min, x.max, by = 0.25)#0.25
    y_user_werte <- seq(y.min, y.max, by = 0.25)#0.25
    ...
    #print(paste("Länge von areax:", length(x_user_werte)))
    #print(paste("Länge von areay:", length(y_user_werte)))
    ...
    RGRIDY_metrics <- OccurrenceReachedGrid
      ...
      for (i in 1:(length(df_basis$methods)-1)) {
        for (j in (i+1):length(df_basis$methods)) {
          ...
          {
            target_size_IMP <- length(v_M1_IMP)  # Stichprobengröße von Grafik-Daten
            target_size_OCC <- length(v_M1_OCC)
            v_M2_IMP <- sample(v_M2_IMP, target_size_IMP, replace = TRUE) # Sample oder Bootstrap
            v_M2_OCC <- sample(v_M2_OCC, target_size_OCC, replace = TRUE)
          ...
          { 
            ...
            new.x.test <- AndersonDarlingTest(v_M1_IMP,"pnorm", mean = pooling.x, sd = pooling.sd_x)
...
scenfile <- paste0("myapp/pictures/paper2/distances_pValue_Risks.xlsx")

write.csv(gerundeter_df, file = paste0("myapp/pictures/paper2/distances_pValue_Risks.csv"), row.names = TRUE)
...
scentext <- paste0("Streumaße")
scenfile <- paste0("myapp/pictures/paper2/distances_aggregate1_pValue_Risks.xlsx")

write.csv(summary_df, file = paste0("myapp/pictures/paper2/distances_aggregate1_pValue_Risks.csv"), row.names = TRUE)
...
scentext <- paste0("Streumaße")
scenfile <- paste0("myapp/pictures/paper2/distances_aggregate2_pValue_Risks.xlsx")

write.csv(new_summary_df, file = paste0("myapp/pictures/paper2/distances_aggregate2_pValue_Risks.csv"), row.names = TRUE)
...
scenfile <- paste0("myapp/pictures/paper2/weighted_pValue_Risks.xlsx")

write.csv(special.distance.df, file = paste0("myapp/pictures/paper2/weighted_pValue_Risks.csv"), row.names = TRUE)
