...
all.answers <- all.answers %>% filter(QUES_ID %in% c("281", "283"))
...
#Graphischer Wert über Flächen & REACHED GRID TEIL & WEIGHT VARIANTE
  ...
  ######VERTEILUNGEN
      ...
      #KLASSISCH <-> AREA
      x.test <- (ad.test(classicX_metrics, x.daten$Wert)) 
      y.test <- (ad.test(classicY_metrics, y.daten$Wert)) 
      print (x.test)
      print (str(x.test))
      p_value_version1 <- x.test$ad[1, 3]
      ...
      x1.test <- (ad.test(classicX_metrics, x.center)) 
      y1.test <- (ad.test(classicY_metrics, y.center)) 
      ...
      x.test <- (ad.test(classicX_metrics, graphiccentertoclassicX_metrics)) 
      y.test <- (ad.test(classicY_metrics, graphiccentertoclassicY_metrics)) 
      D.value.x <- x.test$statistic
      value <- (D.value.x[[1]])
      ...
      x.test <- (ad.test(classicX_metrics, graphic_gridreach_X_metrics)) 
      y.test <- (ad.test(classicY_metrics, graphic_gridreach_Y_metrics)) 
      D.value.x <- x.test$statistic
      value <- (D.value.x[[1]])
      ...
      #KLASSISCH <-> WEIGHTED
      x.test <- (ad.test(classicX_metrics, weight.x.simulations)) 
      y.test <- (ad.test(classicY_metrics, weight.y.simulations)) 
      D.value.x <- x.test$statistic
      value <- (D.value.x[[1]])
      ...
      #KLASSISCH <-> POOLING
      simulated_norm_x <- rnorm(length(classicX_metrics), mean = pooling.x, sd = pooling.sd_x)
      simulated_norm_y <- rnorm(length(classicY_metrics), mean = pooling.y, sd = pooling.sd_y)
      x.test <- ad.test(classicX_metrics, simulated_norm_x)
      y.test <- ad.test(classicY_metrics, simulated_norm_y)
      D.value.x <- x.test$statistic
      value <- (D.value.x[[1]])
      ...
      for(i in 1:10000) {
        ...
        x.weighted.classic <- ad.test(classicX_metrics, sample_values.x)
        y.weighted.classic <- ad.test(classicY_metrics, sample_values.y)
        x.weighted.area <- ad.test(areax, sample_values.x)
        y.weighted.area <- ad.test(areay, sample_values.y)
        x.weighted.center <- ad.test(middlegridx, sample_values.x)
        y.weighted.center <- ad.test(middlegridy, sample_values.y)
        x.weighted.centertogrid <- ad.test(graphiccentertoclassicX_metrics, sample_values.x)
        y.weighted.centertogrid <- ad.test(graphiccentertoclassicY_metrics, sample_values.y)
        x.weighted.reachedgrid <- ad.test(graphic_gridreach_X_metrics, sample_values.x)
        y.weighted.reachedgrid <- ad.test(graphic_gridreach_Y_metrics, sample_values.y)
        x.weighted.pooling <- ad.test(sample_values.x, function(x) pnorm(x, mean = pooling.x, sd = pooling.sd_x))
        y.weighted.pooling <- ad.test(sample_values.y, function(x) pnorm(y, mean = pooling.y, sd = pooling.sd_y))
        ...
        p.x.weighted.classic[i] <- x.weighted.classic$ad[1, 3]
        p.y.weighted.classic[i] <- y.weighted.classic$ad[1, 3]
        p.x.weighted.area[i] <- x.weighted.area$ad[1, 3]
        p.y.weighted.area[i] <- y.weighted.area$ad[1, 3]
        p.x.weighted.center[i] <- x.weighted.center$ad[1, 3]
        p.y.weighted.center[i] <- y.weighted.center$ad[1, 3]
        p.x.weighted.centertogrid[i] <- x.weighted.centertogrid$ad[1, 3]
        p.y.weighted.centertogrid[i] <- y.weighted.centertogrid$ad[1, 3]
        p.x.weighted.reachedgrid[i] <- x.weighted.reachedgrid$ad[1, 3]
        p.y.weighted.reachedgrid[i] <- y.weighted.reachedgrid$ad[1, 3]
        p.x.weighted.pooling[i] <- x.weighted.pooling$ad[1, 3]
        p.y.weighted.pooling[i] <- y.weighted.pooling$ad[1, 3]
...
scenfile <- paste0("myapp/files/80_distances/distances_pValue_Risks.xlsx")

write.csv(gerundeter_df, file = paste0("myapp/files/80_distances/distances_pValue_Risks.csv"), row.names = TRUE)
...
scentext <- paste0("Streumaße")
scenfile <- paste0("myapp/files/80_distances/distances_aggregate1_pValue_Risks.xlsx")

write.csv(summary_df, file = paste0("myapp/files/80_distances/distances_aggregate1_pValue_Risks.csv"), row.names = TRUE)
...
scentext <- paste0("Streumaße")
scenfile <- paste0("myapp/files/80_distances/distances_aggregate2_pValue_Risks.xlsx")

write.csv(new_summary_df, file = paste0("myapp/files/80_distances/distances_aggregate2_pValue_Risks.csv"), row.names = TRUE)
...
scenfile <- paste0("myapp/files/80_distances/weighted_pValue_Risks.xlsx")

write.csv(special.distance.df, file = paste0("myapp/files/80_distances/weighted_pValue_Risks.csv"), row.names = TRUE)
