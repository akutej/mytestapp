...
for (anz in 1:number.scenarios) {
  ...
  directory <- "myapp/pictures/42_boxplots"
...
boxplot(actual.df$scaled_uncertainty_middle_Y, main = "OCCURRENCE", ylab = "value", ylim = c(0, 100))
mean_value <- mean(actual.df$scaled_uncertainty_middle_Y)
...
boxplot(actual.df$scaled_uncertainty_middle_X, main = "IMPACT", ylab = "value", ylim = c(0, 100))
mean_value <- mean(actual.df$scaled_uncertainty_middle_X)
points(mean_value, col = "red", pch = 19)
