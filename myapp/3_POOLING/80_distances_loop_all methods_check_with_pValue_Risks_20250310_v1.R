...
#Graphischer Wert über Flächen & REACHED GRID TEIL & WEIGHT VARIANTE
  ...
  for (i in 1:numberofanswers){
    ...
    weight.x.simulations <- sample(weight.x.summary_df$weight.x.scale, size = 10000000, replace = TRUE, prob = weight.x.summary_df$weight.x.Wahrs)#10000000
      ...
      for (i in 1:(length(df_basis$methods)-1)) {
        for (j in (i+1):length(df_basis$methods)) {
          ...
          { 
            ...
            vsimppooling <- rnorm(5000, pooling.x, pooling.sd_x)
            vsoccpooling <- rnorm(5000, pooling.y, pooling.sd_y)
            new.x.test <- wasserstein1d(v_M1_IMP,vsimppooling )
            new.y.test <- wasserstein1d(v_M1_OCC,vsoccpooling)
          ...
          else 
          {
            ...
            new.x.test <- wasserstein1d(v_M1_IMP, v_M2_IMP)
            new.y.test <- wasserstein1d(v_M1_OCC, v_M2_OCC)
          ...
          get.x.value <-  new.x.test       #p_wert.x <- new.x.test$p.value
