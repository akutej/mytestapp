...
mean_x <- dfcalc[anz, "x"]
sd_x <- dfcalc[anz, "x_3xsigma"] / 3

mean_y <- dfcalc[anz, "y"]
sd_y <- dfcalc[anz, "y_3xsigma"] / 3
...
filetitleImpact <- (paste0("myapp/pictures/paper2/",scentext,"_Impact_sc1.png"))
# Histogramm für x-Koordinaten
png(file=filetitleImpact,width=23, height=23,units="cm",res=600)
bar_positions <-barplot(pdf_x, 
                        names.arg = round(x, 2), 
                        main="", 
                        col="lightblue",
                        xlab = "Impact Value", 
                        ...
                        las = 1,        # Dreht die Achsenbeschriftung für bessere Lesbarkeit
                        cex.names = 2   # X-Achsenbeschriftung (Tick-Labels) vergrößern
                        #xaxt="n"        # Verhindert automatische X-Achse
)

# Fügt "Weighted Frequency" links oben hinzu
mtext("Weighted Frequency", side=3, line=2, adj=-0.1, cex=2)
#axis(1, at=seq(min(bar_positions), max(bar_positions), length.out=6), labels=c(0, 20, 40, 60, 80, 100), cex.axis=2)
#axis(1, at=bar_positions, labels=FALSE, lwd=1)  
dev.off()
                        
dev.off()

filetitleImpact1 <- (paste0("myapp/pictures/paper2/",scentext,"_Impact_sc2.png"))
...
png(file=filetitleImpact1,width=23, height=23,units="cm",res=600)
bar_positions <- barplot(pdf_x_scale,
                         names.arg = round (x_scale,2),
                         main="", 
                         col="lightblue",
                         xlab = "Impact Value", 
                         ylab = "", 
...
filetitleOcc <- (paste0("myapp/pictures/paper2/",scentext,"_Occurrence_sc1.png"))
# Histogramm für y-Koordinaten
png(file=filetitleOcc,width=23, height=23,units="cm",res=600)
bar_positions <- barplot(pdf_y, 
                         names.arg = round (y,2),
                         main="", 
                         col="lightblue",
...
filetitleOcc1 <- (paste0("myapp/pictures/paper2/",scentext,"_Occurrence_sc2.png"))
...
png(file = filetitleOcc1,width=23, height=23,units="cm",res=600)
bar_positions <- barplot(pdf_y_scale,
                         names.arg = round (y_scale,2),
                         main="", 
                         col="lightblue",
                         xlab = "Probability of Occurrence Value", 
                         ylab = "", 
...
iqr_x <- q3_x - q1_x
minimum_x <- mean_x - 3 * sd_x
maximum_x <- mean_x + 3 * sd_x
...
iqr_y <- q3_y - q1_y
minimum_y <- mean_y - 3 * sd_y
maximum_y <- mean_y + 3 * sd_y
...
boxplot_stats_x <- list(stats = matrix(c(minimum_x, q1_x, median_x, q3_x, maximum_x), nrow=5), n=1)
boxplot_stats_y <- list(stats = matrix(c(minimum_y, q1_y, median_y, q3_y, maximum_y), nrow=5), n=1)

directory <- "myapp/pictures/paper2"
