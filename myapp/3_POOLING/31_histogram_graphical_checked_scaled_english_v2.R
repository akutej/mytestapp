...
dfall <- dfall %>% filter(QUES_ID == "401")
...
for (anz in 1:numberscenarios) {
  ...
  filetitleImpact <- (paste0("myapp/pictures/31_histogramms_graphical_eot_bin/",scentext,"- Impact.png"))
  filetitleOcc <- (paste0("myapp/pictures/31_histogramms_graphical_eot_bin/",scentext,"- Occurrence.png"))
  ...
  hist(IMPACT,breaks = bin_grenzen, main="", xlab = "", ylab = "Frequency", xlim = c(0, 100), col = "lightblue", border = "black")
  dev.off()

  png(file=filetitleOcc,width=1500, height=1000, res=150)
  hist(LIKELIHOOD,breaks = bin_grenzen, main = "", xlab = "", ylab = "Frequency", xlim = c(0, 100), col = "lightblue", border = "black")
  dev.off()
