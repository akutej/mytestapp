...
#Graphischer Wert über Flächen & REACHED GRID TEIL & WEIGHT VARIANTE
  ...
  ######VERTEILUNGEN
      ...
      #KLASSISCH <-> AREA
      D.value.x <- wasserstein1d(classicX_metrics,x.daten$Wert) 
      D.value.y <- wasserstein1d(classicY_metrics,y.daten$Wert)
      print ("KLASSISCH <-> AREA")
      print (D.value.x)
      print (D.value.y)
      ...
      print ("KLASSISCH <-> CENTER")
      print (x1.test)
      print (y1.test)
      D.value.x <- x1.test$statistic
      value <- D.value.x[[1]]
