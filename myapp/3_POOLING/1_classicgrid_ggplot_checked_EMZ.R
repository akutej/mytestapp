...
answerstable <- read_excel("myapp/EMZ/raw/correct_new.xlsx")
...
print(thisis)
scenfile <- (paste0("myapp/EMZ/answers/", scentext,"_",actualtype,".png"))  
...
for (i in 1:nrow(df_sorted)) {
  ...
  image_file <- paste0("myapp/EMZ/answers/Scenario ", scenario$Scenario_number, "_", scenario$Scenario_type, ".png")
...
print(doc, target = "myapp/EMZ/risikoanalyse_bericht.docx")

wb <- createWorkbook()
addWorksheet(wb, "Risiken")
writeData(wb, sheet = "Risiken", x = df_sorted)
saveWorkbook(wb, file = "myapp/EMZ/auswertung_risiken_sortiert.xlsx", overwrite = TRUE)
