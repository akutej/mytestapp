...
for (anz in 1:number.scenarios) {
  ...
  filetitleImpact <- (paste0("myapp/pictures/100_weight_boxplot/",actualscenario,"_Impact.png"))
  filetitleOcc <- (paste0("myapp/pictures/100_weight_boxplot/",actualscenario,"_Occurrence.png"))
  
  simulations.x <- sample(x.summary_df$x.scale, size = 10000000, replace = TRUE, prob = x.summary_df$x.Wahrs)
  simulations.y <- sample(y.summary_df$y.scale, size = 10000000, replace = TRUE, prob = y.summary_df$y.Wahrs)
  
  
  
  
  png(filetitleImpact, width = 200, height = 500)
  boxplot(simulations.x, main = "IMPACT", ylab = "value", ylim = c(0, 100))
  mean_value <- mean(simulations.x)
  points(mean_value, col = "red", pch = 19)
  dev.off()  
  
  
  png(filetitleOcc, width = 200, height = 500)
  boxplot(simulations.y, main = "OCCURRENCE", ylab = "value", ylim = c(0, 100))
  mean_value <- mean(simulations.y)
  points(mean_value, col = "red", pch = 19)
