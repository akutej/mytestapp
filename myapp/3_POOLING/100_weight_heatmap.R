...
for (anz in 1:number.scenarios) {
 ...
 #print(x.summary_df)
  ...
  filetitleImpact <- (paste0("myapp/pictures/100_weight_heatmap/",actualscenario,"_heatmap.png"))
  
  
  simulations.x <- sample(x.summary_df$x.scale, size = 1000000, replace = TRUE, prob = x.summary_df$x.Wahrs)
  simulations.y <- sample(y.summary_df$y.scale, size = 1000000, replace = TRUE, prob = y.summary_df$y.Wahrs)
  
  df <- data.frame(x = simulations.x, y = simulations.y)
