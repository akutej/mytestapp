...
rectangleg1$col <- rgb(red = 1, green = 0, blue = 0, alpha = rectangleg1$weight_rescaled)
rectangleg2$col <- rgb(red = 0, green = 0, blue = 1, alpha = rectangleg2$weight_rescaled)
...
thisis <- ggplot() + 
  ...
  xlim(-60,400) +
  ylim(0,460) +
  geom_rect(data = rectangleg1, aes(xmin = x1g1, xmax = x2g1, ymin = y1g1, ymax = y2g1, color = col), fill = NA)+
  geom_rect(data = rectangleg2, aes(xmin = x1g2, xmax = x2g2, ymin = y1g2, ymax = y2g2, color = col), fill = NA)+
...
print (thisis)
scenfile <- (paste0("myapp/pictures/2_graphicweight/", scentext,"_rectangle.png"))  
