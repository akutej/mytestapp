...
for (anz in 1:1){#number.scenarios) {
  ...
  barplot(x.grouped$x, names.arg = x.grouped$Group, main = "", xlab = "", ylab = "Frequency", space = 0, col = "lightblue")# xaxt = "n")
  dev.off()
  
  png(file=filetitleOcc,width=1500, height=1000, res=150)
  barplot(y.grouped$x, names.arg = y.grouped$Group, main = "", xlab = "", ylab = "Frequency", space = 0, col = "lightblue")#, xaxt = "n")
  dev.off()
  ...
  simulations <- sample(x.summary_df$x.scale, size = 500000000, replace = TRUE, prob = x.summary_df$x.Wahrs)
