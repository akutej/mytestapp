...
#Graphischer Wert über Flächen & REACHED GRID TEIL & WEIGHT VARIANTE
  ...
  for (i in 1:numberofanswers){
    ...
    weight.x.simulations <- sample(weight.x.summary_df$weight.x.scale, size = 500000, replace = TRUE, prob = weight.x.summary_df$weight.x.Wahrs)
    weight.y.simulations <- sample(weight.y.summary_df$weight.y.scale, size = 500000, replace = TRUE, prob = weight.y.summary_df$weight.y.Wahrs)#500000000
  ...
  ######VERTEILUNGEN
      ...
      name_DF_IMP <- c("classicX_metrics", "x.daten$Wert", "actual.df$scaled_uncertainty_middle_X", "graphiccentertoclassicX_metrics", "graphic_gridreach_X_metrics", "weight.x.simulations", "pooling.x")
      name_DF_IMP2 <- c("-","-","-","-","-","-","pooling.sd_x")
      name_DF_OCC <- c("classicY_metrics", "y.daten$Wert", "actual.df$scaled_uncertainty_middle_Y", "graphiccentertoclassicY_metrics", "graphic_gridreach_Y_metrics", "weight.y.simulations", "pooling.y")
      ...
      for (i in 1:(length(df_basis$methods)-1)) {
        for (j in (i+1):length(methods)) {
          ...
          methode2 <- df_basis$methods[j]
          value_M2_IMP <- df_basis$variableIMP[j]
          print ("Dies ist ein Pärchen:")
          print (methode1)
          print (value_M1_IMP)
          print (value_M1_IMP2)
          print (value_M1_OCC)
