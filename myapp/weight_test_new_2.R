...
for (anz in 1:1) {#89:89) { #
  ...
  for (i in 1:numberofanswers){
    ...
    df_prod <- df_prod[order(df_prod$value), , drop = FALSE]
    
    
    while (weight.actualvaluex <= actualmax.x){
      
      
      #weight.transformedx <- (1*weight.IMPACT.value)
      #print (transformedx)
      
      thisvalue <- user_data$weight
      df_prod$value[df_prod$scale == weight.actualvaluex] <- thisvalue
    ...
    #sorted_df <- df_prod[order(df_prod$value), ]
  ...
  bin_grenzen <- seq(from = 0, to = 100, by = 0.25)
  
  
  
  

  grouped <- aggregate(summary_df$sum, by = list(Group = cut(summary_df$scale, breaks = seq(0, 100, by = 5), include.lowest = TRUE)), FUN = sum)
  
  print (grouped)
  
  
  #hist(summary_df$Histo,breaks = bin_grenzen, main="", xlab = "", ylab = "Frequency", xlim = c(0, 100), col = "lightblue", border = "black")
  # Erzeugung des Histogramms mit barplot

  
    barplot(summary_df$Histo, names.arg =summary_df$scale, main = "Histogramm", xlab = "X-Skala", ylab = "Summierte Y-Werte")

    
    # Erstelle das Balkendiagramm mit ggplot2
  #ggplot(summary_df, aes(x=scale, y=Histo)) +
  #  geom_bar(stat="identity", fill="skyblue") +
  #  labs(title="Histogramm", x="X-Skala", y="Summierte Y-Werte")
